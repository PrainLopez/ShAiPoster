...
export const getPosts = query({
  ...
  handler: async (ctx, args) => {
    const posts = await ctx.db.query('posts').order('desc').take(args.count);
...
export const addBlueSkyPostFromUrl = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.scheduler.runAfter(0, internal.post.patchBlueSkyPost, {
      id: args.id,
      did: uri,
      text: text,
...
export const addPostFromUrl = mutation({
  ...
  handler: async (ctx, args) => {
    const post = await ctx.db
      .query('posts')
      .withIndex('byOriginUrl', (q) => q.eq('originUrl', args.postUrl))
      .first();
    if (post) {
      return {
        message: 'exists',
    ...
    await ctx.scheduler.runAfter(0, internal.post.addBlueSkyPostFromUrl, {
      id: postId,
